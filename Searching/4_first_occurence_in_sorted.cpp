// Find the index of first occurence of element in sorted array by iteration
/*
...
int iterativeFirstOcc(int arr[], int n, int ele)
{
    int low = 0, high = n - 1;
    while (low <= high)
    {
        ...
        else
        {
            if (mid == 0 || arr[mid - 1] != arr[mid]) //If element is at first position or previous element is not equal to current mid then it is first occurence
                ...
                high = mid - 1;  // Else go in previous part to find first occurence if above condition fails
        }
    }
    return -1;
}
...
int main()
{
    ...
    int ele = 10;
    int val = iterativeFirstOcc(arr, n, ele);
    if (val != -1)
